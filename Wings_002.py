...
def parse_obj_by_object(obj_path):
    """Return dict: {object_name: [(x,y,z), ...]} from an OBJ file."""
    objects = {}
    current = None
...
def compute_all_panels(obj_path, wing_dihed_deg, log_func=print):
    ...
    required = ["Wing1", "Wing2", "Horizontal_Stab", "Vert_Stab"]
    missing = [name for name in required if name not in objs]
    if missing:
        raise ValueError(f"Missing objects in OBJ: {missing}")

    results = {}

    # Wings and H-Stab span along X
    for name in ["Wing1", "Wing2", "Horizontal_Stab"]:
        info = section_chord_info(objs[name], span_axis="x")
        if name in ["Wing1", "Wing2"]:
            info["dihed_deg"] = float(wing_dihed_deg)
        else:
            info["dihed_deg"] = 0.0  # horizontal stab dihedral
        results[name] = info
        log_func(
    ...
    # Vertical stab spans along Y, chord is still Z
    vinfo = section_chord_info(objs["Vert_Stab"], span_axis="y")
    vinfo["dihed_deg"] = 90.0  # PM convention: vertical tail as 90° dihedral
