...
class PMGeneratorGUI:
    ...
    def apply_dark_theme(self):
        """
        Apply a simple dark theme to the whole window.
        Non-destructive: we just tweak colors on whatever widgets exist.
        """
        BG_MAIN = "#1e1e1e"   # main background (VS Code-ish)
        BG_FRAME = "#252526"  # frames
        BG_INPUT = "#3c3c3c"  # entries, listboxes, text
        FG_TEXT = "#e0e0e0"   # primary text
        FG_SUBTLE = "#aaaaaa" # secondary text
        ACCENT = "#007acc"    # accent (buttons border / active)
        ...
        def style_widget(w):
            ...
            try:
                if cls in ("Frame", "Labelframe"):
                    ...
                    # Keep titles slightly brighter, others slightly dimmer
                    text = str(w.cget("text"))
                    fg = FG_TEXT if text else FG_SUBTLE
                    w.configure(bg=BG_FRAME, fg=fg)
                elif cls in ("Button",):
                    w.configure(
                        bg=BG_INPUT,
                        fg=FG_TEXT,
                ...
                # some widgets may not accept some options – ignore quietly
                pass
    ...
    def _run_new_aircraft(self, obj_path: str) -> None:
        ...
        # Apply dark theme at the very end
