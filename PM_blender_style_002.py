...
def build_body_block_from_template(
    ...
    # --- Walk template line by line and fill values ---
    for line in template_lines:
        stripped = line.strip()
...
if __name__ == "__main__":
    ...
    print_bodies_summary(bodies)
    print()

    print_station_z_list(bodies, body_index)
    print()

    #print_ring(bodies, body_index, station_i)
    #print()

    # print_station_PMstyle(bodies, body_index, station_i)
    # print()
    # For example, build body/0 full block using the template
    write_body_block_from_template_to_file(
        bodies,
        body_index,
        template_path=TEMPLATE_PATH,
    )
    # NEW: full body block (header + geo_xyz)
    #print_body_block_PMstyle(bodies, body_index)
    #print()

    # And save to a file
    #write_body_block_to_file(bodies, body_index)

    debug_station_z_spread(bodies, body_index)
